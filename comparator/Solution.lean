...
theorem tt_bounded_error_parameterized 
  ...
  change abs ((List.foldl (fun acc mat => acc * mat) (1 : Matrix (Fin rank_bound) (Fin rank_bound) ℝ) [cores 0 (x 0)]) ⟨0, h_rb⟩ ⟨0, h_rb⟩ - target) ≤ ε
...
theorem tensor_rank_complexity_scaling 
  ...
  have h_step1 : d * (r ^ 2) * G ≤ d * (G ^ 2) * G := Nat.mul_le_mul_right G h_inner
...
theorem master_update_contraction 
  ...
  · intro y hy
    have h_eq := ContractingWith.eq_or_edist_eq_top_of_fixedPoints hf (ContractingWith.fixedPoint_isFixedPt hf) hy
    cases h_eq with
    | inl h => exact h.symm
    | inr h => 
      have hd_ne : edist (ContractingWith.fixedPoint T hf) y ≠ ⊤ := edist_ne_top _ _
      exact (hd_ne h).elim
